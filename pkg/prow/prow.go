...
func (as *ArtifactScanner) Run() error {
	...
	if as.config.ProwJobID != "" {
		...
		pjURL = as.config.ProwJobID
		klog.Infof("got the prow job URL: %s", pjURL)
	} else if as.config.ProwJobURL != "" {
		pjURL = as.config.ProwJobURL
		klog.Infof("got the prow job URL: %s", pjURL)

		jobTarget, err = determineJobTargetFromProwURL(pjURL)
		if err != nil {
			return fmt.Errorf("failed to determine job target: %+v", err)
		}
	} else {
		return fmt.Errorf("ScannerConfig doesn't contain neither ProwJobID nor ProwURL")
	}
...
func determineJobTargetFromProwURL(prowURL string) (jobTarget string, err error) {
	if strings.Contains(prowURL, "pull-ci-redhat-appstudio-infra-deployments") {
		// prow URL is from infra-deployments repo
		jobTarget = "appstudio-e2e-tests"
	} else if strings.Contains(prowURL, "pull-ci-redhat-appstudio-e2e-tests") {
		// prow URL is from e2e-tests repo
		jobTarget = "redhat-appstudio-e2e"
	} else {
		return "", fmt.Errorf("unable to determine the target from the prowURL: %s", prowURL)
	}
